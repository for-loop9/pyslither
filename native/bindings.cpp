...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<env>(m, "Simulation")
      .def(py::init([](float rad, float bs, float str, float ms, float ts,
             ...
             if (!env_init(e)) {
               printf("Environment initialization failed: number of maximum parts exceeds %d.\n", MAX_PARTS);
               exit(-1);
             }
      ...
      .def("set_snake_target_angle",
           [](env* e, float ang, int i) { e->snake.tang[i] = ang; }, py::arg("angle"), py::arg("i"),
           "Set the desired heading for snake `i` in radians (`0` to `2pi`).")
      .def("set_snake_boost",
           [](env* e, bool b, int i) { e->snake.b[i] = b; },
      ...
      .def("get_snake_id_array",
           [](env* e) {
             int* ptr = e->snake.id;
             ssize_t n = tdarray_length(ptr);
             return py::memoryview::from_buffer(
                 ptr, sizeof(int), py::format_descriptor<int>::value, {n}, {sizeof(int)}, true);
           }, "Unique IDs of all snakes.")
      .def("get_snake_kill_count_array",
           [](env* e) {
             int* ptr = e->snake.kc;
             return py::array_t<int>(tdarray_length(ptr), ptr);
           }, "Kill counts of all snakes.")
      .def("get_snake_num_parts_array",
           [](env* e) {
             int* ptr = e->snake.np;
             ssize_t n = tdarray_length(ptr);
             return py::memoryview::from_buffer(
      ...
      .def("get_snake_radius_array",
           [](env* e) {
             float* ptr = e->snake.rad;
             return py::array_t<float>(tdarray_length(ptr), ptr);
           }, "Collision radii of all snakes.")

      .def_property_readonly("num_food",
           [](env* e) { return tdarray_length(e->food.x); },
           "Number of foods currently in the world.")
      .def("get_food_x_array",
           [](env* e) {
             float* ptr = e->food.x;
             return py::array_t<float>(tdarray_length(ptr), ptr);
           }, "X coordinates of all foods.")

      .def("get_food_y_array",
           [](env* e) {
             float* ptr = e->food.y;
             return py::array_t<float>(tdarray_length(ptr), ptr);
           }, "Y coordinates of all foods.")
      .def("get_food_value_array",
           [](env* e) {
             float* ptr = e->food.v;
             return py::array_t<float>(tdarray_length(ptr), ptr);
           }, "Values of all foods.")
      ...
      .def("get_segment_x0",
           [](env* e, int i) { return e->snake.px[e->csnake.sidx[i]][e->csnake.s0[i]]; },
           py::arg("i"), "Start X of collision segment `i`.")
      .def("get_segment_y0",
           [](env* e, int i) { return e->snake.py[e->csnake.sidx[i]][e->csnake.s0[i]]; },
           py::arg("i"), "Start Y of collision segment `i`.")
      .def("get_segment_x1",
           [](env* e, int i) { return e->snake.px[e->csnake.sidx[i]][e->csnake.s1[i]]; },
           py::arg("i"), "End X of collision segment `i`.")
      .def("get_segment_y1",
           [](env* e, int i) { return e->snake.py[e->csnake.sidx[i]][e->csnake.s1[i]]; },
           py::arg("i"), "End Y of collision segment `i`.")
      .def("get_segment_snake_idx",
           [](env* e, int i) { return e->csnake.sidx[i]; },
           py::arg("i"), "Index of the snake that owns segment `i`.")

      .def("get_segment_cell",
           [](env* e, float x, float y) {
             int cell = (int)(y * e->csnake.icsz) * e->csnake.gsz + (int)(x * e->csnake.icsz);
      ...
      .def("get_food_cell",
           [](env* e, float x, float y) {
             int cell = (int)(y * e->cfood.icsz) * e->cfood.gsz + (int)(x * e->cfood.icsz);
